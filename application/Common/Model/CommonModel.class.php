...
class CommonModel extends Model {
    ...
    public function getList($field='*',$condition=array(),$order=array(),$limit=''){
        $query = $this->getQuery($field,$condition,$order,$limit);
        $list = $query->select();
        return $list;
    }
    protected function getQuery($field='*',$condition=array(),$order=array(),$limit=''){
        $where = array(
            'status'=>1
        );
        if($condition){
